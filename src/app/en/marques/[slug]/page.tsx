...
export default async function BrandPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <Container className="py-12">
      <Link href="/en/marques" className="flex items-center gap-1.5 text-sm font-medium text-steel hover:text-copper">
        <ArrowLeft size={15} /> All brands
      </Link>

      <div className="mt-6 flex flex-wrap items-start justify-between gap-6">
        <div>
          <Eyebrow>{brand.countryEn}</Eyebrow>
          <h1 className="mt-3 font-display text-2xl font-semibold text-blueprint sm:text-3xl">
            {brand.name}
          </h1>
          <p className="mt-1 font-mono text-[12px] uppercase tracking-[0.08em] text-copper">{brand.taglineEn}</p>
          <p className="mt-4 max-w-xl text-base leading-relaxed text-steel">{brand.descriptionEn}</p>
          {brand.website && (
            <a
              href={brand.website}
              target="_blank"
            ...
            >
              {brand.name} official website <ExternalLink size={14} />
            </a>
          )}
        </div>
        <div className="flex h-20 w-20 shrink-0 items-center justify-center border border-steel-soft/40 p-2">
          {brand.logoUrl ? (
            ...
            <img src={assetPath(brand.logoUrl)} alt={`${brand.name} logo`} className="h-full w-full object-contain" />
          ) : (
      ...
      {brand.photoUrls && brand.photoUrls.length > 0 && (
        <div className={`mt-8 grid gap-3 ${brand.photoUrls.length === 1 ? "" : "sm:grid-cols-3"}`}>
          {brand.photoUrls.map((url, i) => (
            ...
            >
              <Image src={assetPath(url)} alt={`GAT and ${brand.name} on site`} fill className="object-cover" />
            </div>
          ))}
        </div>
      )}
      ...
      <div className="mt-12">
        <SectionDivider label={`${brand.name} products`} />
        {brandProducts.length > 0 ? (
          ...
          <p className="mt-6 text-sm text-steel">
            The detailed catalogue for this brand is being finalised — contact us for a specific
            product.
          </p>
        )}
      </div>
